...
class Pokemon {
  ...
  constructor(name: string, type: string, level: number, attacks: string[]) {
    ...
    this.type = type;
    this.level = level;
    this.attacks = attacks; // Los ataques deben de ser un arreglo de strings
  }
  ...
  clone(): Pokemon {
    return new Pokemon(
      ...
      this.type,
      this.level,
      [...this.attacks] // Clonamos el arreglo de ataques para evitar referencias
    );
...
function main() {
  ...
  // Verificamos que el Pokémon base no ha sido modificado
  console.log("\nPokémon Base después de clonar:");
  basePokemon.displayInfo();
}
main();
