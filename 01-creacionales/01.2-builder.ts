...
class QueryBuilder {
  ...
  execute(): string {
    const sql = [];
    sql.push(`Select ${this.fields.join(", ")} from ${this.table}`);
    if (this.conditions.length > 0) {
      sql.push(`where ${this.conditions.join(" and ")}`);
    }
    if (this.orderFields.length > 0) {
      sql.push(`order by ${this.orderFields.join(", ")}`);
    }
    if (this.limitCount !== undefined) {
      sql.push(`limit ${this.limitCount}`);
    }
    return sql.join(" ");
  }
}
