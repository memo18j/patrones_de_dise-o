...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(
        `Supervisor: %cAprobando solicitud de compra de $${amount}`,
        COLORS.green
      );
      return;
    }
    ...
    console.log(
      `Supervisor: Solicitud de compra de $${amount} es demasiado alta, pasando al siguiente aprobador.`
    );
    this.next(amount);
  }
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(
        `Manager: %cAprobando solicitud de compra de $${amount}`,
        COLORS.green
      );
      return;
    }
    ...
    console.log(
      `Manager: Solicitud de compra de $${amount} es demasiado alta, pasando al siguiente aprobador.`
    );
    this.next(amount);
  }
...
class Director extends BaseApprover {
  // TODO: Implementar el método approveRequest si el monto es menor o igual a 10000
  override approveRequest(amount: number): void {
    if (amount <= 10000) {
      console.log(
        `Director: %cAprobando solicitud de compra de $${amount}`,
        COLORS.green
      );
      return;
    }
    ...
    console.log(
      `Director: Solicitud de compra de $${amount} es demasiado alta, pasando al siguiente aprobador.`
    );
    this.next(amount);
  }
}
